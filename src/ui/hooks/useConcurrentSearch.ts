...
export interface ConcurrentSearchState {
  ...
  perSource: Partial<Record<SourceId, SourceState>>;
  loading: boolean;
  done: number;
  total: number;
}
...
function blankPerSource(loading: boolean): Partial<Record<SourceId, SourceState>> {
  const out: Partial<Record<SourceId, SourceState>> = {};
...
// SeedDeck's default ordering: healthiest first. The results view can re-sort
