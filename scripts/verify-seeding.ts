/**
 * Proves SeedDeck's seeding actually transfers bytes (not just a UI label), and
...
async function main(): Promise<void> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "seeddeck-seedcheck-"));
  ...
  // drop it. This is what SeedDeck now captures at download time and seeds from.
